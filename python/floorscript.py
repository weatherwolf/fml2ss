...
from he import point_in_polygon, remove_colinear_points, order_points_nearest
...
#     polygon = []
#     print(f"room {room.id} walls: {len(room.walls)}")
#     for wall in room.walls:
#         if (wall.a_x, wall.a_y) not in polygon:
#             polygon.append((wall.a_x, wall.a_y))
#             print(f"wall {wall.id} a: {wall.a_x}, {wall.a_y}")
#         if (wall.b_x, wall.b_y) not in polygon:
#             polygon.append((wall.b_x, wall.b_y))
#             print(f"wall {wall.id} b: {wall.b_x}, {wall.b_y}")

#     # STEP 1: remove colinear points (in current order)
#     polygon = remove_colinear_points(polygon)

#     # STEP 2: reorder points by nearest neighbor path
#     polygon = order_points_nearest(polygon)
...
def make_design_json(design, snap_value=None, decimals=2, parent_design_id=None):
    ...
    for room_id, wall_ids in rooms_dict.items():
        ...
        if len(room.walls) >= 3:
            ...
            print(f"room {room_id} area: {room.area}")
            print(f"polygon: {polygon}")
            print('\n')
    ...
    for i, item in enumerate(design.items):
        ...
        print(f"\nProcessing item {item_obj.refid} at position ({item_obj.position_x}, {item_obj.position_y})")
        ...
        for room in design_obj.rooms:
            if len(room.walls) >= 3:
                ...
                print(f"  Testing against room {room.id} with {len(room.walls)} walls")
                print(f"  Room polygon: {polygon}")
                print(f"  Item polygon: {item_obj.position_x}, {item_obj.position_y}")
